...
public abstract class AbstractRedisSentinelBase extends VertxTestBase {
    ...
    protected final static void createRedisSentinelInstance(final Integer sentinelPort, final Integer masterPort)
        ...
        System.out.println("master host: " + host);

        redis.embedded.RedisSentinel sentinel = redis.embedded.RedisSentinel.builder()
                .redisExecProvider(TestEnvConfig.redisExecProvider)
                .port(sentinelPort)
    ...
    protected final static void createRedisInstance(final Integer port, final String... settings) throws Exception {
        ...
        RedisServerBuilder builder = RedisServer.builder().redisExecProvider(TestEnvConfig.redisExecProvider).port(port);
